...
if (strlen($_SESSION['tid']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $staffid = $_SESSION['tid'];
        $newpassword=$_POST['newpassword'];
        
        $sql = mysqli_query($con, "SELECT * FROM teachertbl WHERE tid='".$_SESSION['tid']."'");
        if ($row = mysqli_fetch_array($sql)) {
            ...
            if (strpos($newpassword,'@')==false && strpos($newpassword,'%')== false) {
                ...
                $updatePass = mysqli_query($con, "UPDATE teachertbl SET password='$newpassword' WHERE tid='$staffid'");
